...
- **One-command self-host** — `docker compose up` starts db, redis, GROBID, MinIO, API, and frontend
...
- `docker-compose.yml` — Full-stack orchestration (db, redis, grobid, minio, api, frontend)
- `Makefile` — `make release` gated on `make eval` (PASS required before tagging)

### Infrastructure
- PostgreSQL 16 + pgvector, Redis 7, GROBID 0.8.1, MinIO
- SQL migrations (idempotent, docker-entrypoint-initdb.d)
- CI: ruff check → mypy core → lint-imports → pytest
- `make eval` — gate must PASS before `make release` creates the v0.1.0 tag
